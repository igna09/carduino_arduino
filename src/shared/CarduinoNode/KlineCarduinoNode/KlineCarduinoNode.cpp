...
KlineCarduinoNode::KlineCarduinoNode(uint8_t pin_rx, uint8_t pin_tx, int cs, int interruptPin, char *ssid,  char *password) : CarduinoNode(cs, interruptPin, ssid,  password) {
	...
	instrumentBlock1.addValueToRead(ValueToRead(ValueToReadEnum::VALUE_TO_READ_0, [&](float v){
		//send value with category id car status and message relative to block/index
		Serial.print("Read instrument ecu block 1 value 0");
	}));
	instrumentBlock1.addValueToRead(ValueToRead(ValueToReadEnum::VALUE_TO_READ_2, [&](float v){
		//send value with category id car status and message relative to block/index
		Serial.print("Read instrument ecu block 1 value 2");
	}));
	...
	engineBlock1.addValueToRead(ValueToRead(ValueToReadEnum::VALUE_TO_READ_0, [&](float v){
		//send value with category id car status and message relative to block/index
		Serial.print("Read engine ecu block 1 value 0");
	}));
	...
	engineBlock2.addValueToRead(ValueToRead(ValueToReadEnum::VALUE_TO_READ_1, [&](float v){
		//send value with category id car status and message relative to block/index
		Serial.print("Read engine ecu block 2 value 1");
	}));
	engineBlock2.addValueToRead(ValueToRead(ValueToReadEnum::VALUE_TO_READ_3, [&](float v){
		//send value with category id car status and message relative to block/index
		Serial.print("Read engine ecu block 2 value 3");
	}));

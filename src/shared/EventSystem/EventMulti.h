...
public:
    ...
    void printValue(Print& out, bool serialMode = false) const override {
        if(!serialMode) out.print('[');
...
private:
    ...
    // Stampa
    template<size_t... I>
    void printImpl(Print& out, bool serialMode, std::index_sequence<I...>) const {
        if constexpr (sizeof...(I) > 0) {
            bool first = true;
            const char* separator = serialMode ? ";" : ", ";
            ((out.print(first ? "" : separator),
              out.print(std::get<I>(values)),
              first = false), ...);
            if(serialMode) out.print(separator);
        }
    }
};

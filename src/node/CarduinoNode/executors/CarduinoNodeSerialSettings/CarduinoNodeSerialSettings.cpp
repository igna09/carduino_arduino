...
CarduinoNodeSerialSettings::CarduinoNodeSerialSettings() : CarduinoNodeExecutorInterface(EventCategory::SETTINGS) {};

void CarduinoNodeSerialSettings::execute(CarduinoNode *node, Message *message) {
    this->node = node;
...
void CarduinoNodeSerialSettings::getSettings() {
    for (auto &kv : node->settings) {
        ...
        node->sendSerialMessage(m);
    }
}

void CarduinoNodeSerialSettings::writeSetting(Message *m) {

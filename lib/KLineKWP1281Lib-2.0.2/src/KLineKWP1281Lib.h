...
//If the following line is commented out will be used function void pointer instead of std::function
#define STD_FUNCTION_SUPPORTED
...
{  
  public:
    ...
    #ifdef STD_FUNCTION_SUPPORTED
    using callBack_type             = std::function<void()>;                                                                            //generic callback (for custom functions)
    using beginFunction_type        = std::function<void(unsigned long baud)>;                                                          //beginFunction
    using endFunction_type          = std::function<void()>;                                                                            //endFunction
    using sendFunction_type         = std::function<void(uint8_t data)>;                                                                //sendFunction
    using receiveFunction_type      = std::function<bool(uint8_t &data)>;                                                               //receiveFunction
    using KWP1281debugFunction_type = std::function<void(bool type, uint8_t sequence, uint8_t command, uint8_t* data, uint8_t length)>; //debugFunction
    #else
